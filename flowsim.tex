...
\subsection{\protect{\emph{classifier}.specifier}}
...
\item[-classifier] \cmdarg{classifier} is a procedure which receives a
flow creation upcall when a new lower level flow is created.  This
allows the \cmdarg{classifier} to determine the correct flow type and
class for the upper level flow associated with the lower level flow.
\item[-flows]  \cmdarg{flows} is a complex data structure (expressed
as a string!) which describes the various upper level flows and the
default behaviour to be associated with those flows.  \cmdarg{flows}
...
\end{description}
If the \cmdarg{name} subparameter is given, the upper level flows are
...
\cmd{fsim\_start\_enumeration} (see~\ref{sec:fsim_start_enumeration}).
...
whether the header fields in \cmdarg{ulflows} and
